...
import graphGUI.model.Assistant;
...
public class Controller
{
    ...
    public void addEdge()
    {
        if(view.getAddEdge().isSelected())
        {
            ...
            {
                Vertex v1 = Assistant.FIND_CLOSEST_VERTEX(graph, event1.getX(), event1.getY());
                try
                {
                    ...
                    else
                    {
                        ...
                        view.getCenter().setOnMouseClicked(event2 ->
                        {
                            try
                            {
                                Vertex v2 = Assistant.FIND_CLOSEST_VERTEX(graph, event2.getX(), event2.getY());
    ...
    public void removeVertex()
    {
        if(view.getRemoveVertex().isSelected())
        {
            ...
            view.getCenter().setOnMouseClicked(event ->
            {
                try
                {
                    Vertex vertex = Assistant.FIND_CLOSEST_VERTEX(graph, event.getX(), event.getY());
        ...
        if(view.getRemoveEdge().isSelected())
        {
            ...
            view.getCenter().setOnMouseClicked(event ->
            {
                try
                {
                    ...
                    for(Edge edge : edges)
                    {
                        double disOfEdge = Assistant.DO_All(edge, event.getX(), event.getY());
                        if(disOfEdge <= dis)
                        {
                            deleteEdge = edge;
                            dis = disOfEdge;
    ...
    public void moveVertex()
    {
        if(view.getMoveVertex().isSelected())
        {
            ...
            view.getCenter().setOnMousePressed(event1 ->
            {
                try
                {
                    Vertex vertex = Assistant.FIND_CLOSEST_VERTEX(graph, event1.getX(), event1.getY());
    ...
    public void numOfConnCompo()
    {
        ...
        else
        {
            ...
            final String content = "Total number of Connected Components in the graph is: "
                    + Assistant.TOTAL_COMPONENTS(graph);
            AlertBox.DisplayText(header, content);
        }
    }
    ...
    public void numOfCutVertices()
    {
        ...
        else
        {
            ...
            final String content = "Total number of Cut Vertices in the graph is: "
                    + Assistant.TOTAL_CUT_VERTICES(graph);
            AlertBox.DisplayText(header, content);
        }
    }
    ...
    public void showCutVertices()
    {
        ...
        int components = Assistant.TOTAL_COMPONENTS(graph);
        for(Vertex vertex : graph.getAllVertices())
        {
            Set<Edge> edges = graph.getEdges(vertex);
            graph.removeVertex(vertex);
            if(components < Assistant.TOTAL_COMPONENTS(graph)) cutVertices.add(vertex);
            Assistant.ADD_BACK(graph, vertex, edges);
        }
    ...
    public void generateGraph()
    {
        ...
        if(graph.totalVertices() <= 0)
        {
            graph = Assistant.GENERATE_RANDOM_GRAPH();
